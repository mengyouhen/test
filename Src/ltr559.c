...
*******************************************************************************/
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id ltr559_i2c_id[] = {{"ltr559",0},{}};
...
static int ltr559_i2c_read_reg(u8 regnum)
{
	...
	if(res <= 0)
	{	   
	   printf("read reg send res = %d\n",res);
		return res;
	}
	res = i2c_master_recv(ltr559_obj->client, reg_value, 0x1);
	if(res <= 0)
	{
		printf("read reg recv res = %d\n",res);
		return res;
	}
...
static void ltr559_power(struct alsps_hw *hw, unsigned int on) 
{
	...
	if(hw->power_id != POWER_NONE_MACRO)
	{
		if(power_on == on)
		{
			printf("ignore power control: %d\n", on);
		}
		else if(on)
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "ltr559")) 
			{
				printf("power on fails!!\n");
			}
		}
		else
		{
...
static int ltr559_ps_enable(struct i2c_client *client, int enable)
{
	...
	printf("ltr559_ps_enable() ...start!\n");

	regdata = ltr559_i2c_read_reg(LTR559_PS_CONTR);
	if (enable != 0) {
		printf("PS: enable ps only \n");
		regdata |= 0x03;
	}
	else {
		printf("PS: disable ps only \n");
		regdata &= 0xfc;
	}
	...
	if (err < 0)
	{
		printf("PS: enable ps err: %d en: %d \n", err, enable);
		return err;
	}
	mdelay(WAKEUP_DELAY);
	...
	if (0 == ltr559_obj->hw->polling_mode_ps && enable != 0)
	{
		...
		if (err < 0)
		{
			printf("ltr559_dynamic_calibrate() failed\n");			
		}
#endif
		ltr559_ps_set_thres();
	}
...
static int ltr559_ps_read(struct i2c_client *client, u16 *data)
{
	...
	printf("ps_rawdata_psval_lo = %d\n", psval_lo);
	if (psval_lo < 0){	    
	    printf("psval_lo error\n");
		psdata = psval_lo;
		goto out;
	}

	psval_hi = ltr559_i2c_read_reg(LTR559_PS_DATA_1);
    printf("ps_rawdata_psval_hi = %d\n", psval_hi);
	if (psval_hi < 0){
	    printf("psval_hi error\n");
		psdata = psval_hi;
		goto out;
	}
	...
	*data = psdata;
    printf("ltr559_ps_read: ps_rawdata = %d\n", psdata);
...
static int ltr559_dynamic_calibrate(void)
{
	...
	if (noise < 100) {
		...
		ps_thd_val_low  = 1400;
		printf("dynamic calibrate fails!!\n");
	}
	...
	printf("%s:noise = %d\n", __func__, noise);
	printf("%s:obj->ps_thd_val_high = %d\n", __func__, ps_thd_val_high);
	printf("%s:obj->ps_thd_val_low = %d\n", __func__, ps_thd_val_low);
...
static int ltr559_als_enable(struct i2c_client *client, int enable)
{
	...
	if (enable != 0) {
		printf("ALS(1): enable als only \n");
		regdata |= 0x01;
	}
	else {
		printf("ALS(1): disable als only \n");
		regdata &= 0xfe;
	}
	...
	if (err < 0)
	{
		printf("ALS: enable als err: %d en: %d \n", err, enable);
		return err;
	}
...
static int ltr559_als_read(struct i2c_client *client, u16* data)
{
	...
	printf("ltr559_als_read: als_value_lux = %d\n", luxdata_int);
out:
	*data = luxdata_int;
...
static int ltr559_get_ps_value(struct ltr559_priv *obj, u16 ps)
{
	...
	{
		//invalid = 1;
		printf("ligh too high will result to failt proximiy\n");
		return 1;  /*far away*/
	}
...
static ssize_t ltr559_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		printf("invalid content: '%s', length = %zu\n", buf, count);
	}
	return count;    
}
/*----------------------------------------------------------------------------*/
...
EXIT_ERR:
	printf("ltr559_check_and_clear_intr fail\n");
	return 1;
}
...
static void ltr559_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		printf("ltr559_eint_work: rawdata ps=%d!\n",obj->ps);
		value = ltr559_get_ps_value(obj, obj->ps);
		printf("intr_flag_value=%d\n",intr_flag_value);
		if(intr_flag_value){
			databuf[0] = LTR559_PS_THRES_LOW_0;	
			databuf[1] = (u8)((atomic_read(&obj->ps_thd_val_low)) & 0x00FF);
...
int ltr559_setup_eint(struct i2c_client *client)
{
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		printf("Cannot find alsps pinctrl!\n");
	}
	
	pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");
	if (IS_ERR(pins_cfg)) {
		ret = PTR_ERR(pins_cfg);
		printf("Cannot find alsps pinctrl pin_cfg!\n");
	}
...
/*----------------------------------for factory mode test---------------------------------------*/
		...
		//case ALSPS_CALIBRATE_PS:    //ps 校准
			//ps_calibration();
			//break;

		//case ALSPS_WRITE_CALIBRATE:     //获取 ps校准数据
...
static int ltr559_init_client(void)
{
	...
	printf("LTR559_PS setgain = %d!\n", init_ps_gain);
	res = ltr559_i2c_write_reg(LTR559_PS_CONTR, init_ps_gain);
	if (res<0)
	{
		printf("ltr559 set ps gain error\n");
		goto EXIT_ERR;
	}	
	...
	if (res<0)
	{
		printf("ltr559 set ps pulse error\n");
		goto EXIT_ERR;
	}
	...
	if (res<0)
	{
		printf("ltr559 set ps led error\n");
		goto EXIT_ERR;
	}
...
{
	int res = 0;
	printf("ltr559_obj als enable value = %d\n", en);
	...
	if (res) {
		printf("als_enable_nodata is failed!!\n");
		return -1;
	}
	return 0;
}
...
{
	int res = 0;
	printf("ltr559_obj ps enable value = %d\n", en);
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printf("ltr559_init_client() start...!\n");
	...
	printf("ltr559_init_client() OK!\n");
	
	err = misc_register(&ltr559_device);
	if(err)
	{
		printf("ltr559_device register failed\n");
	...
	if(err)
	{
		printf("register light batch support err = %d\n", err);
	}
	
	err = batch_register_support_info(ID_PROXIMITY,ps_ctl.is_support_batch, 1, 0);
	if(err)
	{
		printf("register proximity batch support err = %d\n", err);
	}
...
static int ltr559_i2c_remove(struct i2c_client *client)
{
	...
	if(err)
	{
		printf("ltr559_delete_attr fail: %d\n", err);
	}

	err = misc_deregister(&ltr559_device);
	if(err)
	{
		printf("misc_deregister fail: %d\n", err);    
	}
